...
Intel4 = cdf.loc[cdf['ID']=='IN_01_T_002']
...
lenVectorB = len(Beta)
GenFunValE = [[0.0]*lenVectorA]*lenVectorB
GenFunValC = [[0.0]*lenVectorA]*lenVectorB
ListErrro = list()
ListCurre = list()
for i in range(lenVectorA):
    for ii in range(lenVectorB):
        ...
        print("Alpha %f Beta %f Current %f Error %f  " % (Alpha[i],Beta[ii],meanConsumption, Errorvector))
    print(Alpha[i])
    if (i == 0):
        df = pd.DataFrame(data={"0": ListErrro})
        df.to_csv("./GeneralErrorFileH1.csv", sep=',', index=False)
        df = pd.DataFrame(data={"0": ListCurre})
        df.to_csv("./GeneralCurrentFileH1.csv", sep=',', index=False)
    else:
        VarCsv = pd.read_csv("GeneralErrorFileH1.csv")
        VarCsv[str(Alpha[i])] = ListErrro
        VarCsv.to_csv("./GeneralErrorFileH1.csv", sep=',', index=False)
        VarCsv = pd.read_csv("GeneralCurrentFileH1.csv")
        VarCsv[str(Alpha[i])] = ListCurre
        VarCsv.to_csv("./GeneralCurrentFileH1.csv", sep=',', index=False)
    ListErrro.clear()
    ListCurre.clear()
