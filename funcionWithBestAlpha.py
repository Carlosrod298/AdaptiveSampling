...
for i in range(0,minutesTotal,30):
    ...
    if dataWindow['HSU'].var(ddof=0) > 0:
        VarValues30M.append(dataWindow['HSU'].var(ddof=0))
    else:
        VarValues30M.append(0.001);
...
Alpha = 4.6e-3
Beta =  0
FSampling = getSampling(VarValues30M,Alpha,Beta)
Psample = [int(round(1 / (60 * x))) for x in FSampling]
Psample1 = [int(round(3623/(x*1000))) for x in VarValues30M]
